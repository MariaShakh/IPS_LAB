...
duration<double> duration_for_ser;
duration<double> duration_for_par;
...
void SerialGaussMethod( double **matrix, const int rows, double* result )
{
	...
	duration_for_ser = (t2 - t1);
	printf("Duration for direct evaluation in Serial gauss method: %lf\n\n", duration_for_ser);
...
int main()
{
	...
	*/
	...
	printf("Speed-up = %f\n", duration_for_ser.count() / duration_for_par.count());
